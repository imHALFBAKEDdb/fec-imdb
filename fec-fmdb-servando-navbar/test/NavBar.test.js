import React from "react";
import ReactDOM from "react-dom";
import Enzyme from "enzyme";
import { shallow } from "enzyme";
import Adapter from "enzyme-adapter-react-16";
import NavBar from "../client/components/NavBar.jsx";

Enzyme.configure({ adapter: new Adapter() });

it("Should render without crashing", () => {
  const div = document.createElement("div");
  ReactDOM.render(<NavBar />, div);
});

it("Search dropdown appears when we clicked on it", () => {
  const wrapper = shallow(<NavBar />);
  wrapper.find(".searchdropdown").simulate("click");
  expect(wrapper.find(".searchdropdown")).toBeTruthy();
});

it("Navigation dropdowns appear when the mouse hovers over an option", () => {
  const wrapper = shallow(<NavBar />);
  wrapper
    .find(".dropdowncontent")
    .first()
    .simulate("hover");
  expect(wrapper.find(".dropdowncontent")).toBeTruthy();
});

it("Renders the images with correct src", () => {
  const wrapper = shallow(<NavBar />);
  expect(wrapper.find(".weblogo").prop("src")).toEqual(
    "data:image/jpeg;base64,"
  );
  expect(wrapper.find(".imdbpro").prop("src")).toEqual(
    "https://m.media-amazon.com/images/G/01/imdb/images/navbar/imdbpro_logo_nb-3000473826._CB470041611_.png"
  );
  expect(
    wrapper
      .find(".sociallogo")
      .first()
      .prop("src")
  ).toEqual(
    "https://iconmonstr.com/wp-content/g/gd/makefg.php?i=../assets/preview/2012/png/iconmonstr-facebook-5.png&r=255&g=255&b=255"
  );
  expect(
    wrapper
      .find(".personallogo")
      .first()
      .prop("src")
  ).toEqual(
    "https://www.freeiconspng.com/uploads/facebook-sign-in-button-png-26.png"
  );
});
